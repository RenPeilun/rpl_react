...
import * as Utils from '../utils/AllUtil'
import 'whatwg-fetch'
...
export default store => next => action => {
  ...
  let {endpoint, method, contentType, dataType, payload, secured, success, fail, types} = apiCall
  ...
  next(actionWith({ type: requestType, data: payload }));
  ...
  return fetch(`${HttpProps.API_ROOT}/${endpoint}`, {
          ...
          .then(data=>{
            ...
            if(data.code && data.code !== null) {
              next(actionWith({type: failureType, error: {code: data.code, message: data.message}}))

              if(fail)
                 fail({code: data.code, message: data.message})

               return
            }

            next(actionWith({type: successType, data: data.data}));
            if(success)
              success(data.data)
            return 
          })
          .catch(err=>{
            ...
            next(actionWith({type: failureType, error: {code: err.code, message: message}}))

            if(fail){
              fail({code: err.code, message: message})
            }
          })
}